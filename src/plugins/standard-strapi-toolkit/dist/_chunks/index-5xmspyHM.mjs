...
const multiSelectRegisterAdmin = {
  ...
  components: {
    Input: async () => import("./input-Dy6uz3A1.mjs")
  },
  options: {
    base: [
      {
...
const colorPickerRegisterAdmin = {
  ...
  icon: Icon$1,
  components: {
    Input: async () => import("./input-D8JBcxI6.mjs")
  }
};
...
const iconPickerRegisterAdmin = {
  ...
  icon: Icon,
  components: {
    Input: async () => import("./input-zh120hkl.mjs")
  }
};
const index = {
  register(app) {
    app.addMenuLink({
      ...
      Component: async () => {
        const { App } = await import("./App-BBJGKeju.mjs");
        return App;
      }
    });
...
//# sourceMappingURL=index-5xmspyHM.mjs.map
